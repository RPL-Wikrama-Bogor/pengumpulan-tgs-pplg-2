...
// simpan data ke db : 'name_column' => $request->name_input
        Medicine::create([
            'name' => $request->name, //pertama disesuaikan dengan inout, kedua sesuai  model
            'type' => $request->type,
            'price' => $request->price,
            'stock' => $request->stock,
        ]);
...
// simpan data ke db : 'name_column' => $request->name_input
    ...
    public function updateStock(Request $request, $id)
    {
        ...
        // ambil data sblm update, untuk dibandingkan
        $medicineBefore = Medicine::where('id', $id)->first();
        if ($request->stock <= $medicineBefore['stock']) {
            //jika stock yang di input <= stock sebelumnya, kirim format error
