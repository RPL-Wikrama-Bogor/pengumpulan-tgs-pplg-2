<div class="container">
    ...
    <table class="table table-light table-striped table-bordered">
        ...
        <?php foreach ($data['barang'] as $row) : ?>
            ...
            <td>
                <?php if ($row['status'] == 'Sudah Kembali') : ?>
                <a href="<?= BASE_URL ?>/barang/hapus/<?= $row['id'] ?>" class="btn btn-danger" onclick="return confirm('Yakin Anda Menghapus data?');"><i class="las la-trash"></i></a>
                <?php else : ?>
                <a href="<?= BASE_URL ?>/barang/edit/<?= $row['id'] ?>" class="btn btn-primary"><i class="las la-edit"></i></a>
                <a href="<?= BASE_URL ?>/barang/hapus/<?= $row['id'] ?>" class="btn btn-danger" onclick="return confirm('Yakin Anda Akan Menghapus data?');"><i class="las la-trash"></i></a>
                <?php endif; ?>
            </td>
            </tr>
        <?php $no++;
