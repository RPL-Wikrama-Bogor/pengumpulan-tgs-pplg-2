...
@section('content')
    <form action="{{ route('order.store') }}" class="card p-4 mt-5" method="POST">
        ...
        <div class="mb-3 d-flex align-items-start">
            <label for="name_costumer" class="form-label" style="width: 12%"> Penanggung Jawab :</label>
            <p class="" style="width: 85% margin-top:  "> <b>{{ Auth::user()->name }}</b></p>
        </div>

        <div class="mb-3 d-flex align-items-center">
            <label for="name_costumer" class="form-label" style="width: 12%"> Nama Pembeli : </label>
            <input type="text" name="name_costumer" id="nama_costumer" class="form-control" style="width: 88%">
        </div>
