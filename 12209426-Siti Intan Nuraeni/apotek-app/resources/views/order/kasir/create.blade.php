...
@section('content')
     <form action="{{ route('order.store') }}" class="card p-4 mt-5" method="POST">
        ...
        <div class="mb-3">
            ...
            {{-- name dnegan [] biasanya dipake buat column yang tipe datanya json/array, dan biasanya digunakan apabila input dengan tujuan data yang sama ada banyak (dan dari banyak input yang datanya sama tsb, datanya akan diambil seluruhnya dalam bentuk array)--}}
            ...
            {{-- karena akan ada js yang menampilkan select ketika di klik, maka sediakan tempat penyimpanan element yang akan dihasilkan dari JS tsb --}}
            <div id="wrap-select" class="mt-3"></div>
            <p class="text-primary" style="margin-left: 12%; margin-top: 15px; cursor: pointer;" onclick= "addSelect()">+Tambah Pesanan</p>
        </div>
     ...
     @push('script')
        ...
        // gunakan query untuk memanggil html tempat el baru akan ditambahkan
        // append : menambahkan el html dibagikan bawah sebelum penutup tag terkait
        $("#wrap-select").append(el);
        // agar no pesanan bertambah sesuai jumlah terkait
        // mengubah isinya menggunakan html
        no++;
            }
        </script>
        @endpush